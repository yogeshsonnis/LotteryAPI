...
{
    public class CloseShiftController : ApiController
    {
        LotteryBlankDatabaseEntities context;
        //CloseShiftController obj = new CloseShiftController();
        ...
        [Route("api/CloseShift/GetSoldOutHistory")]
        public IEnumerable<SoldOut_TicketInfo> GetHistory()
        {
            ...
            var data = (from s in context.tblSoldOut_History select s).ToList();
            foreach (var v in data)
            {
                LotteryHistory.Add(new SoldOut_TicketInfo
                {
                    EmployeeID = Convert.ToInt16(v.EmployeeId),
                    ShiftID = Convert.ToInt16(v.Shift_Id),
                    No_of_Tickets_Sold = Convert.ToInt16(v.No_of_Tickets_Sold)
        ...
        //    context = new LotteryBlankDatabaseEntities();
        //    var SoldHistory = (from s in context.tblSoldOut_History where s.EmployeeId == data.EmployeeID && s.SoldOut_Date == System.DateTime.Today select s).ToList();
        //    var SoldOut = (from s in context.tblSoldouts where s.EmployeeId == data.EmployeeID && s.Created_On == System.DateTime.Today select s).ToList();
        //    var result = from bl in SoldHistory
        //                 join activel in SoldOut
        //                 on bl.Packet_Id equals activel.Packet_Id into a
        //                 from b in a.DefaultIfEmpty(new tblSoldout())
        //                 select new
        //                 {
        //                     b.Box_No,
        //                     bl.EmployeeId,
        ...
        //                     b.Packet_Id,
        //                     b.PackPosition_Open,
        //                     b.PackPosition_Close,
        //                     b.Price,
        //                     bl.No_of_Tickets_Sold,
        //                     b.Game_Id,
        //                     b.Created_On,
        //                 };
        //    if (result != null)
        //    {
        //        var abc = result.Where(x => x.EmployeeId == data.EmployeeID).ToList();
        ...
        //                Game_Id = i.Game_Id,
        //                Total_Price = Convert.ToInt32(i.Price) * i.No_of_Tickets_Sold
        ...
        [Route("api/CloseShift/NewGetAllRecord")]
        public HttpResponseMessage NewGetAllRecord([FromBody] SoldOut_TicketInfo data)
        {
            SoldOutHistory = new ObservableCollection<SoldOut_TicketInfo>();
            context = new LotteryBlankDatabaseEntities();
            if (data.ShiftID == 0)
            ...
            {
                var x = (from s in context.tblShifts where s.StoreId == data.Store_Id && s.Date == data.Created_Date
                         && s.EndTime == data.CloseTime select s).ToList().FirstOrDefault();
        ...
        [Route("api/CloseShift/NewGetDailyReportRecord")]
        public HttpResponseMessage NewGetDailyReportRecord([FromBody] Activate_Ticket data)
        {
            SoldOutHistory = new ObservableCollection<SoldOut_TicketInfo>();
            context = new LotteryBlankDatabaseEntities();
        ...
        [Route("api/CloseShift/CreatePDF")]

        public HttpResponseMessage CreatePDF([FromBody] SoldOut_TicketInfo data)
        {
            int temp = 0;
            //string filename = "ABC";
            ...
            string bookPath_Pdf = @"C:\Projects\storeyogidemo\Pdf";

            //string path = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);  //find root path
            //DirectoryInfo directory = new DirectoryInfo(bookPath_Pdf);
            // var PdfPath = path + @"\DailyPDF";
            Directory.CreateDirectory(bookPath_Pdf);

            var renderedReportFile = System.IO.Path.Combine(bookPath_Pdf, System.IO.Path.GetFileName(filename));

            renderer.PdfDocument.Save(renderedReportFile);
            //Process.Start(renderedReportFile);

            HttpResponseMessage result = new HttpResponseMessage(HttpStatusCode.OK);
            //FileStream fileStream = File.OpenRead(renderedReportFile);
        ...
        //public void SavePdfData(int tempPdfStoreId, DateTime tempPdfDate)
        //{
        //    pdfStoreId = tempPdfStoreId;
        //    pdfDate = tempPdfDate;
        //}
        private void AddHistoryEventsTableToReport(Section section, ParagraphFormat formatBold, ParagraphFormat formatNormal)
        {
            ...
            NewGetDailyReportRecord(activeObj);
            ...
            NewGetTerminalDataRecord(terminalClsObj);
           ...
           // int shortOver = 0;
            ...
            activeObj.Created_Date = pdfDate;
            activeObj.EmployeeID = 0;
            NewGetDailyActivateRecord(activeObj);
            ...
            NewGetDailySoldOutRecord(soldOutObj);
        ...
        /// <returns></returns>
        [Route("api/CloseShift/NewGetTerminalDataRecord")]
        public HttpResponseMessage NewGetTerminalDataRecord([FromBody] Terminal_Details data)
        {
            ...
            if (Shift != null)
            {
                ...
                else if (data.ShiftID == 0 && data.EmployeeID == 0) // Hamburger Daily report
                {
                    var x = (from s in context.tblShifts
                             where s.StoreId == data.Store_Id && s.Date == data.Date select s).ToList();
                ...
                {
                    var x = (from s in context.tblShifts where s.StoreId == data.Store_Id && s.Date == data.Date
                             && s.EndTime == data.CloseTime select s).ToList();
        ...
        [Route("api/CloseShift/NewGetDailyActivateRecord")]
        public HttpResponseMessage NewGetDailyActivateRecord([FromBody] Activate_Ticket data)
        {
        ...
        [Route("api/CloseShift/NewGetDailySoldOutRecord")]
        public HttpResponseMessage NewGetDailySoldOutRecord([FromBody] SoldOut_TicketInfo data)
        {
            LotteryHistory = new ObservableCollection<SoldOut_TicketInfo>();
            context = new LotteryBlankDatabaseEntities();
            if (data.EmployeeID != 0)
        ...
        [Route("api/CloseShift/ShiftReportCreatePDF")]

        public HttpResponseMessage ShiftReportCreatePDF([FromBody] SoldOut_TicketInfo data)
        {
            int temp = 1;
            ...
            //isFileInUse = FileInUse();

            //// Then you can do some checking
            //if (isFileInUse)
            //    Console.WriteLine("File is in use");
            //else
            //    System.IO.File.Delete(@"C:\Projects\storeyogidemo\Pdf\ShiftReportSample.pdf");

            string filename = "ShiftReport.pdf";
            string bookPath_Pdf = @"C:\Projects\storeyogidemo\Pdf";
        ...
        private void ShiftReportCreateReportPage(Document document)
        {
            try
            {
                ...
                formatNormal.Font.Size = 7;
                //ShiftReportAddHistoryEventsTableToReport(section, formatBold, formatNormal);
        ...
        private void ShiftReportAddHeaderToReport(Section section, ParagraphFormat formatBold, ParagraphFormat formatNormal, int count)
        {
            ...
            var e = (from s in context.tblShifts
                     where s.StoreId == pdfStoreId && s.Date == pdfDate && s.ShiftID == pdfShiftId
                     select s).ToList().FirstOrDefault();
            ...
            NewGetTerminalDataRecord(terminalClsObj);
            var result = GetTerminalDataCollection.Where(x => x.ShiftID == pdfShiftId && x.Date == pdfDate).LastOrDefault();
            ...
            NewGetShiftActivateRecord(activeObj);
            ...
            NewGetSoldoutRecord(soldOutObj);
        ...
        private void ShiftReportgetHeaderDetails(int pdfStoreId, DateTime pdfDate)
        {
            ...
            {
                var shift = (from s in context.tblShifts where s.StoreId == pdfStoreId && s.EmployeeId == pdfEmpId && s.ShiftID == pdfShiftId && s.Date == pdfDate select s).ToList().FirstOrDefault();
        ...
        [Route("api/CloseShift/NewGetShiftActivateRecord")]
        public HttpResponseMessage NewGetShiftActivateRecord([FromBody] Activate_Ticket data)
        {
            LotteryHistoryActivate = new ObservableCollection<Activate_Ticket>();
            context = new LotteryBlankDatabaseEntities();
            if (data.ShiftID == 0)
            ...
            var closeticket = (from s in context.tblClose_Box
                               ...
                               s.Created_On == sdate   select s).ToList();
        ...
        [Route("api/CloseShift/NewGetSoldoutRecord")]
        public HttpResponseMessage NewGetSoldoutRecord([FromBody] SoldOut_TicketInfo data)
        {
        ...
        [Route("api/CloseShift/NewGetReturnRecord")]
        public HttpResponseMessage NewGetReturnRecord([FromBody] Return_TicketInfo data)
        {
            LotteryHistoryReturn = new ObservableCollection<Return_TicketInfo>();
            context = new LotteryBlankDatabaseEntities();
            if (data.ShiftID == 0)
            ...
            else
            {
                ...
                foreach(var r in x)
                {
                    var result = (from s in context.tblReturntickets
                                  ...
                                  && s.Created_On == data.Created_Date  select s).ToList();
